...
export const AmountController: FC<{
  ...
  return (
    <div className={`flex gap-4 py-[9px] px-[13px] border-[1px] border-gray-300 bg-white rounded-md ${className}`}>
      <Button className="p-0" variant="text" onClick={() => onAmountChange(amount - 1)}>
        {amount === 1 ? <TrashIcon /> : <MinusIcon />}
      </Button>
      <div className="flex gap-2">
        <Typography className="text-sm leading-5 font-medium text-gray-700">{amount}</Typography>
        <Typography className="text-sm leading-5 font-medium text-gray-400">{label}</Typography>
      </div>
