export { default as AddImageIcon } from './add_image.svg'
export { default as CheckIcon } from './check.svg'
export { default as ChevronUpIcon } from './chevron-up.svg'
export { default as ChevronDownIcon } from './chevron-down.svg'
export { default as ChevronLeftIcon } from './chevron-left.svg'
export { default as ChevronRightIcon } from './chevron-right.svg'
export { default as CompletedIcon } from './completed.svg'
export { default as GeoIcon } from './geo.svg'
export { default as NonCheckedIcon } from './non_checked.svg'
export { default as PlusIcon } from './plus.svg'
export { default as MinusIcon } from './minus.svg'
export { default as MenuIcon } from './menu.svg'
export { default as ProfileIcon } from './profile.svg'
export { default as CabinetIcon } from './cabinet.svg'
export { default as LogoIcon } from './logo.svg'
export { default as EyesOpenIcon } from './eyes-open.svg'
export { default as EyesClosedIcon } from './eyes-closed.svg'
export { default as SearchIcon } from './search.svg'
export { default as NotificationIcon } from './notification.svg'
export { default as FilterOrderIcon } from './filter-order.svg'
export { default as ListIcon } from './list.svg'
export { default as RectangleIcon } from './rectangle.svg'
export { default as CrossIcon } from './cross.svg'
export { default as ThinCrossIcon } from './thin-cross.svg'
export { default as HeartIcon } from './heart.svg'
export { default as OrderIcon } from './order.svg'
export { default as StarIcon } from './star.svg'
export { default as DeliveryIcon } from './delivery.svg'
export { default as AddIcon } from './add.svg'
export { default as TrashIcon } from './trash.svg'
export { default as BasketIcon } from './basket.svg'
export { default as PaymentIcon } from './payment.svg'
export { default as ArrowLeftIcon } from './arrow-left.svg'
export { default as ArrowRightIcon } from './arrow-right.svg'
export { default as ExitIcon } from './exit.svg'
export { default as DeleteIcon } from './delete.svg'
export { default as AcceptIcon } from './accept.svg'
export { default as RejectIcon } from './reject.svg'
export { default as CommentIcon } from './comment.svg'
export { default as CrossBigIcon } from './cross-big.svg'
export { default as StepperActive } from './stepper-active.svg'
export { default as SuccessIcon } from './success.svg'
